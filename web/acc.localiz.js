...
// fill form field
	if (o.app.state.dataParsedDC != false) {
		if (a.state.needZoom != false && a.state.waitMap == false) { // zoom
	...
	// зумировать и кадрировать, выделить линки и узлы:
			{	sLastError = ''; // select
				...
				for (var i = 0; i < a.arrLockers.length; i++) {
					...
					if (mapAddToSelected(a.map, ln, to.id, act) == false) {window.status = 'Не выделены некоторые обьекты из найденных';};
				}
...
// тут нужно не просто установить статус. тут нужно вызвать метод очистки результатов счета, а он установит статус:
...
DBG('onOpenReportButton \n');
	...
	if (a.state.haveDisconn == false) {
		alert('Сначала нужно определить запорные устройства и отключенных потребителей (кнопка "локализовать").'); return;
	}
	if (a.state.waitServer != false) {
		alert('Необходимо дождаться окончания текущего запроса. \n Подождите.'); return;
	}
...
alert('Сбой обработки запроса AJAX (http status: '+xmlhttp.status+'). \n Рекомендация: \n нажмите CTRL-R');
		} // end http err
...
//	в процессе обработки озадачили карту и надо дождаться пока isBusy() == false:
	...
	this.sethaveFailedLinks = function(val) {
		this.changed = true;
		if (val == false) { this.haveDisconn = false; } else { ; }
		this.haveFailedLinks = val; return this.haveFailedLinks;
	}
	this.sethaveDisconn = function(val) { this.haveDisconn = val; return this.haveDisconn;}
	this.setwaitServer = function(val) {
DBG('state.setwaitServer: ['+val+'] \n');
		this.waitServer = val; return this.waitServer;
	}
	this.sethaveMap = function(val) {
DBG('state.sethaveMap: ['+val+'] \n');
		this.haveMap = val; return this.haveMap;
	}
...
DBG('app.parseXmlFailedLink \n');
	var a = this;
	if (a == null || a.state == null) { // alert('parseXmlFailedLink: I need App object !!!');
		a = getApp();
	}
...
// don't work: var arrQry = xml.getElementsByTagName('/page/query');
...
//		if (t.linkID.toString() == fl.linkID.toString()) {
			bExists = true; break;
		}
	}
	if (bExists == false) {
...
DBG('app.parseXmlLockersAndDisconn \n');
	var a = this;
	if (a == null || a.state == null) { // alert('parseXmlLockersAndDisconn: I need App object !!!');
		a = getApp();
	}
/*
xml parameters:
...
DBG('AJAX:queryGET 1: url: ['+url+'] \n');
	if ( this.isBusy() ) {
		alert('AJAX: Дождитесь окончания текущего запроса.'); return false;
	}
...
function mapAddToSelected(map, sLayerName, sKey, action) { // action: add or remove
